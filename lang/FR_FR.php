<?php
//Formulaire de connexion
define('FORM_MAIL','Adresse mail : ');
define('FORM_LOGIN','Identifiant : ');
define('FORM_PASSWORD','Mot de passe : ');
define('FORM_LOGIN_SUBMIT','Se connecter');
define('LOGIN_TITLE','Connexion');
//Formulaire d'inscription
define('REGISTER_CIVILITY', 'Civilité');
define('REGISTER_TITLE','S\'enregistrer');
define('REGISTER_LASTNAME','Nom :');
define('REGISTER_FIRSTNAME','Prénom :');
define('REGISTER_MAIL','Adresse mail :');
define('REGISTER_LOGIN','Identifiant :');
define('REGISTER_OLDPASSWORD','Ancien mot de passe :');
define('REGISTER_PASSWORD','Nouveau mot de passe :');
define('REGISTER_PASSWORD_VERIFY','Nouveau mot de passe (vérification) :');
define('REGISTER_SUBMIT','Enregistrer');
define('REGISTER_DATE', 'Date de naissance : ');
define('REGISTER_PHONENUMBER', 'Téléphone : ');
define('REGISTER_CITY', 'Ville : ');
define('REGISTER_ADDRESS', 'Adresse : ');
define('REGISTER_POSTALCODE', 'Code postal : ');
//Erreur
define('ERROR_LASTNAME','Erreur dans la saisie du nom');
define('ERROR_FIRSTNAME','Erreur dans la saisie du prénom');
define('ERROR_PASSWORD','Erreur dans la saisie du mot de passe');
define('ERROR_MAIL','Erreur dans la saisie de l\'adresse mail');
define('ERROR_LOGIN','Erreur dans la saisie de l\'identifiant');
define('USER_CONNECTION_SUCCESS','Connecté avec succès');
define('USER_CONNECTION_ERROR','Erreur lors de la connexion');
//Menu
define('NAV_WELCOME','Bienvenue ');
define('NAV_CONNECT','Connexion');
define('NAV_DISCONNECT','Déconnexion');
define('NAV_PROFIL','Profil');
define('NAV_LANGUAGE','<a href="https://st3.depositphotos.com/5183619/16795/v/1600/depositphotos_167959102-stock-illustration-france-flag-vector-round-icon.jpg"');
define('NAV_FRENCH','Français');
define('NAV_ENGLISH','Anglais');
//Modal panier
define('PANER_TITLE', 'Bienvenue dans votre panier');
define('PANER_BODY', 'Votre panier est vide');
define('PANER_CONTINUE', 'Continuer les achats');
define('PANER_NOTCONNECTED', 'Se connecter');

